...
| 控制维度 | Kafka | MQTT | NATS Core | NATS JetStream |
|---------|-------|------|-----------|----------------|
| **流量控制** | 缓冲区+批量发送 | 令牌桶+连接限流 | 待处理消息限制 | 流控+背压 |
| **速率控制** | Producer/Consumer独立控制 | Broker全局控制 | 客户端控制 | Stream速率限制 |
| **资源控制** | JVM堆内存限制 | 连接数+内存限制 | 连接数+内存限制 | 存储空间限制 |
| **自适应控制** | 无（手动调整） | QoS自适应 | 无（自动重连） | 自动扩缩容 |
| **反馈控制** | Consumer Lag监控 | 连接状态监控 | 慢消费者保护 | 消费速率监控 |
| **控制精度** | 高（分区级控制） | 中（连接级控制） | 高（Subject级控制） | 高（Stream级控制） |
