...
| 概念 | Kafka | MQTT | NATS Core | NATS JetStream |
|------|-------|------|-----------|----------------|
| **消息模型** | 发布-订阅+队列 | 发布-订阅 | 发布-订阅+请求响应 | 发布-订阅+流 |
| **主题概念** | Topic（逻辑分类） | Topic（主题树） | Subject（主题字符串） | Subject（流的输入） |
| **分区概念** | Partition（物理分片） | 无 | 无 | Stream（持久化流） |
| **顺序性** | 分区有序 | 无顺序保证 | 无顺序保证 | 流有序 |
| **持久化** | 磁盘+副本 | 可选 | 无 | 文件/S3+副本 |
| **副本机制** | ISR（Leader-Follower） | 无 | 无 | Raft（多副本） |
| **确认机制** | ACK（0/1/-1） | QoS（0/1/2） | 无 | ACK（None/All/Explicit） |
| **消费者组** | Consumer Group | 无 | Queue Group | Consumer（流内） |
...
| 需求 | Kafka | MQTT | NATS Core | NATS JetStream |
|------|-------|------|-----------|----------------|
| **高吞吐量** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| **低延迟** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| **顺序性** | ⭐⭐⭐⭐⭐ | ⭐ | ⭐ | ⭐⭐⭐⭐⭐ |
