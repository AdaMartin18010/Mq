...
1. **存储计算分离**：Broker无状态，BookKeeper负责存储
   - **优势**：Broker可独立扩展，存储层可独立扩展，扩展性强
   - **实现**：Broker不存储消息，消息存储在BookKeeper Ledger中

2. **多租户原生支持**：Tenant → Namespace → Topic三级结构
   - **优势**：原生多租户隔离，适合SaaS场景
   - **实现**：每个Tenant有独立的Namespace，完全隔离

3. **分层存储**：热数据在BookKeeper，冷数据自动卸载到对象存储
   - **优势**：降低存储成本60%，支持PB级数据
   - **实现**：基于TTL和大小策略自动卸载到S3/HDFS

4. **地理复制**：原生支持跨地域复制
   - **优势**：RPO < 1分钟，满足全球化业务需求
   - **实现**：基于BookKeeper的异步复制机制

**性能优化要点**：

- **Broker无状态**：Broker可快速扩展，无需数据迁移
- **BookKeeper顺序写**：Ledger顺序写入，性能优异
- **批量操作**：Producer批量发送，Consumer批量拉取
- **分层存储**：自动卸载冷数据，降低存储成本

## 1.6.5 架构设计对比矩阵

| 架构维度 | Kafka | MQTT | NATS Core | NATS JetStream | Pulsar |
|----------|-------|------|-----------|----------------|--------|
| **分层数** | 6层 | 5层 | 6层 | 7层 | 6层 |
| **中心化程度** | 中心化(Controller) | 中心化(Broker) | 去中心化 | 去中心化 | 中心化(Broker+ZK) |
| **状态管理** | 有状态(分区+副本) | 有状态(会话) | 无状态 | 有状态(RAFT) | 分离(Broker无状态+BookKeeper有状态) |
| **协议复杂度** | 高(二进制) | 中(二进制) | 低(文本) | 低(文本) | 中(二进制) |
| **配置复杂度** | 高(200+参数) | 中(50+参数) | 低(<20参数) | 低(<30参数) | 高(多组件配置) |
| **扩展性** | 水平扩展(分区) | 垂直扩展(连接数) | 水平扩展(节点) | 水平扩展(节点) | 水平扩展(Broker和BookKeeper独立扩展) |
| **故障恢复** | 手动(Controller选举) | 手动(主从切换) | 自动(客户端重连) | 自动(RAFT选举) | 自动(BookKeeper自动恢复) |
| **多租户** | 无 | 无 | 无 | 无 | 原生支持 |
| **分层存储** | 无 | 无 | 无 | 无 | 原生支持 |
...
5. **Pulsar**：适合多租户SaaS、跨地域复制、PB级数据场景
...
- **官方文档**: [Pulsar Architecture](https://pulsar.apache.org/docs/concepts-architecture-overview/)
- **多租户设计**: [Pulsar Multi-tenancy](https://pulsar.apache.org/docs/concepts-multi-tenancy/)
- **BookKeeper设计**: [BookKeeper Architecture](https://bookkeeper.apache.org/docs/4.15.0/overview/)
- **分层存储**: [Pulsar Tiered Storage](https://pulsar.apache.org/docs/concepts-tiered-storage/)
- **地理复制**: [Pulsar Geo-replication](https://pulsar.apache.org/docs/concepts-replication/)
- **性能基准**: [Pulsar Performance](https://pulsar.apache.org/docs/performance/)
