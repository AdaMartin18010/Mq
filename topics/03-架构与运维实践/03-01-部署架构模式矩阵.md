...
| 维度 | Kafka | MQTT | NATS Core | NATS JetStream | Pulsar | 架构决策依据 |
|------|-------|------|-----------|----------------|--------|--------------|
| **部署形态** | 有状态集群 | 有状态集群/单机 | 无状态集群 | 有状态集群 | Broker无状态+BookKeeper有状态 | **关键差异**：Kafka和JetStream必须管理磁盘状态，NATS Core可无状态部署，Pulsar存储与计算分离 |
| **最少生产节点** | 3 Broker+3 ZK | 2 Broker(主从) | 1 Server | 3 Server | 3 Broker+3 BookKeeper+3 ZK | **成本起点**：NATS Core可从单节点起步，Kafka最低需要6个进程，Pulsar需要9个进程 |
| **网络拓扑** | 客户端-Broker直连 | 客户端-Broker直连 | 全网状自发现 | 全网状自发现 | 客户端-Broker直连 | **运维复杂度**：NATS的Gossip协议自动发现，无需负载均衡器；Pulsar Broker无状态，易于扩展 |
| **存储依赖** | 本地磁盘+ZooKeeper | 本地磁盘/Memory | 纯内存 | 本地磁盘 | BookKeeper+ZooKeeper | **可靠性**：Kafka依赖ZK元数据，Pulsar存储与计算分离，Broker故障不影响存储 |
| **云原生适配** | StatefulSet+LocalPV | StatefulSet | DaemonSet/StatefulSet | StatefulSet | Broker无状态+BookKeeper StatefulSet | **K8s部署**：NATS Core适合DaemonSet每个节点部署，Pulsar Broker可无状态部署 |
