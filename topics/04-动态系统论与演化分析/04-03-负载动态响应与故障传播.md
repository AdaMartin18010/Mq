...
- [4.3 负载动态响应与故障传播](#43-负载动态响应与故障传播)
  ...
  - [4.3.3 Pulsar负载响应延迟分析](#433-pulsar负载响应延迟分析)
  - [4.3.4 故障传播动力学](#434-故障传播动力学)
    - [Kafka故障传播链](#kafka故障传播链)
    - [NATS故障隔离机制](#nats故障隔离机制)
    - [Pulsar故障隔离机制](#pulsar故障隔离机制)
  - [4.3.4 动态响应参考资源](#434-动态响应参考资源)
    - [负载响应理论](#负载响应理论)
    - [性能分析参考](#性能分析参考)
...
单点故障：Broker-3宕机
    ↓
直接影响：连接到Broker-3的客户端断开
    ├─ 客户端行为：自动重连到其他Broker（1-5秒）
    └─ 消息行为：BookKeeper继续服务，消息不丢失
    ↓
间接影响：无（Broker无状态）
    ├─ 其他Broker继续工作，无Rebalance
    ├─ BookKeeper Ledger继续服务
    └─ 消息路由自动绕过故障Broker
    ↓
级联效应：被阻断（Isolated）
    └─ BookKeeper故障时，Ledger自动重新复制到其他Bookie

故障传播速度：Broker故障无传播，BookKeeper故障局部影响
传播延迟：即时隔离（客户端重连）
隔离机制：Broker级自动故障转移，BookKeeper级自动恢复

系统动力学模型：
故障影响半径 R(t) = 1 Broker（Broker故障）或 1 Bookie（BookKeeper故障）
恢复时间 TTR = T_重连 + T_追赶 ≈ 5秒（Broker）或 5-10分钟（BookKeeper）
系统可用性 A(t) = (N_Broker-1)/N_Broker × (N_Bookie-1)/N_Bookie × 100% ≈ 99.95%（3 Broker + 3 Bookie）

对比结论：Pulsar的故障域是Broker级（无状态）和BookKeeper级（有状态），存储计算分离降低故障传播
```

**混沌工程验证**：Pulsar混沌实验，随机Kill -9一个Broker节点：

- **恢复时间**：99%客户端在5秒内重连成功
- **吞吐量影响**：瞬降10%，5秒后恢复至95%以上（BookKeeper继续服务）
- **数据完整性**：BookKeeper Ledger自动恢复，无数据丢失

**BookKeeper故障场景**：

- **单Bookie故障**：Ledger自动重新复制到其他Bookie，5-10分钟完成
- **多Bookie故障**：如果故障Bookie数量 < ackQuorum，系统继续服务；否则拒绝写入
- **自动恢复**：BookKeeper自动检测故障Bookie，自动重新复制Ledger
