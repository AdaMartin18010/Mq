...
- [2.5 生产环境案例研究](#25-生产环境案例研究)
  ...
  - [2.5.3 NATS生产案例](#253-nats生产案例)
    ...
    - [Apcera云平台](#apcera云平台)
  - [2.5.4 Pulsar生产案例](#254-pulsar生产案例)
    - [Yahoo消息平台](#yahoo消息平台)
    - [Splunk数据管道](#splunk数据管道)
    - [腾讯云消息队列](#腾讯云消息队列)
  - [2.5.5 案例对比总结](#255-案例对比总结)
...
**公司**: Yahoo（Pulsar的创建者）
**规模**: 单集群处理PB级数据，百万级TPS
**架构**: Pulsar集群（Broker + BookKeeper）

**场景特征**：

- 跨地域消息复制（美国、欧洲、亚洲）
- 多租户SaaS平台（Yahoo Mail、Yahoo Finance等）
- 实时数据管道和流处理
...
Producer层（Yahoo服务）
    ↓
Pulsar Broker集群（无状态）
    ├── Namespace: mail（Yahoo Mail）
    ├── Namespace: finance（Yahoo Finance）
    └── Namespace: ads（广告平台）
    ↓
BookKeeper集群（存储层）
    ├── Ledger复制（3副本）
    ...
    ↓
Consumer层（流处理、分析）
    ├── Pulsar Functions（实时处理）
    └── Flink/Spark（批处理）
```

**关键实践**：

1. **多租户隔离**：每个业务线使用独立的Namespace
2. **地理复制**：跨地域自动复制，RPO < 1分钟
3. **分层存储**：自动将冷数据卸载到S3，降低存储成本60%
4. **统一消息平台**：一个Pulsar集群服务多个业务线
...
- [Yahoo Engineering Blog - Pulsar](https://yahooeng.tumblr.com/post/150078336821/open-sourcing-pulsar-pub-sub-messaging-at-scale)
- [Pulsar at Yahoo Scale](https://pulsar.apache.org/blog/2016/09/06/Pulsar-at-Yahoo-Scale/)
...
**公司**: Splunk
**规模**: 处理数十PB日志数据，千万级消息/天
**架构**: Pulsar + Splunk Enterprise

**场景特征**：

- 日志采集和聚合
- 实时索引和搜索
- 多租户SaaS平台（Splunk Cloud）
...
数据源（应用、服务器、IoT设备）
    ↓
Pulsar集群（消息总线）
    ├── Topic: logs.{tenant}.{source}
    └── Subscription: splunk-indexer
    ↓
Splunk Indexer（消费Pulsar消息）
    ├── 实时索引
    └── 存储到Splunk存储层
    ↓
Splunk Search Head（查询和分析）
```

**关键实践**：

1. **多租户架构**：每个租户使用独立的Namespace和Topic
2. **消息路由**：基于租户ID和数据类型自动路由
3. **弹性扩展**：根据负载动态扩展Broker和BookKeeper节点
4. **数据保留**：基于租户策略设置不同的保留时间
...
- [Splunk Pulsar Integration](https://www.splunk.com/en_us/blog/platform/pulsar-integration.html)
- [Splunk Cloud Architecture](https://www.splunk.com/en_us/blog/platform/splunk-cloud-architecture.html)
...
**公司**: 腾讯云
**规模**: 服务数万企业客户，日均处理千亿级消息
**架构**: 基于Apache Pulsar的云服务
...
1. **云原生架构**：基于Kubernetes部署，自动扩缩容
2. **多租户管理**：每个企业客户完全隔离，资源配额独立
3. **地理复制**：支持跨地域复制，满足全球化业务需求
4. **监控告警**：提供完整的监控和告警能力
...
- [腾讯云TDMQ产品文档](https://cloud.tencent.com/document/product/1179)
- [Apache Pulsar在腾讯云的实践](https://www.infoq.cn/article/pulsar-tencent-cloud)
...
| **Yahoo** | Pulsar | PB级数据、100万+ TPS | P99 < 20ms | 多租户SaaS、跨地域复制 |
| **Splunk** | Pulsar | 数十PB数据、50万+ TPS | P99 < 30ms | 日志管道、多租户SaaS |
| **腾讯云** | Pulsar | 百万级TPS、数万租户 | P99 < 20ms | 企业级消息队列、多租户云服务 |
...
4. **多租户SaaS**: Pulsar是理想选择，提供原生多租户和分层存储
5. **跨地域复制**: Pulsar提供原生地理复制，RPO < 1分钟
6. **混合场景**: 多协议网关（MQTT → Kafka → NATS → Pulsar）是常见模式
